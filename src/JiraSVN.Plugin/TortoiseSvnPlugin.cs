...
namespace JiraSVN.Plugin
{
	...
	{
		private const string GUID = "CF732FD7-AA8A-4E9D-9E15-025E4D1A7E9D";
		private const string CLSID = "{" + GUID + "}";
		private const string BUTTON_TEXT = "{0}";

		private IIssuesService _connector;
		private IIssuesServiceConnection _service;
		private IssuesListView _issues;
		private Configuration _config;
		private bool _cancelled = true;
		private string _commonURL = string.Empty;
		private readonly ConnectingDialog _connectingDialog;
		...
		private string GetAppSetting(string name)
		{
			if (_config == null)
			{
				Log.Verbose("Loading Settings = {0}", GetType().Assembly.Location);
				_config = ConfigurationManager.OpenExeConfiguration(GetType().Assembly.Location);
				Log.Verbose("Loaded");
			}
		...
		public IIssuesService Connector
		{
			get
			{
				if (_connector == null)
				{
					...
					var type = Type.GetType(fullClass, true);
					if (type != null)
						_connector =(IIssuesService)type.InvokeMember("", BindingFlags.CreateInstance, null, null, null);
		...
		private void BackgroundWorkerDoWork(object sender, DoWorkEventArgs args)
		{
			...
			var hParentWnd = (IntPtr)arguments[0];
			var rootUrl = (string)arguments[1];
			var commonRoot = (string)arguments[2];
		...
		private void ShowError(IntPtr hParentWnd, string text, string caption)
		{
			_cancelled = true;
		...
		private string ReadSettings(IntPtr hParentWnd, string serviceUri, ref string userId)
		{
			...
			var pwdDlg = new PasswordEntry(userId, serviceUri);
		...
		private bool TryParseParameters(IntPtr hParentWnd, string parameters, string commonRoot, out string serviceUri,
		...
		private string GetIssueId(IIssue issue, string commonRoot)
		{
		...
		string IBugTraqProvider2.GetCommitMessage2(IntPtr hParentWnd, string parameters, string commonURL, string commonRoot,
		                                           string[] pathList, string originalMessage, string bugID,
		...
		string IBugTraqProvider2.CheckCommit(IntPtr hParentWnd, string parameters, string commonURL, string commonRoot,
		...
		string IBugTraqProvider2.OnCommitFinished(IntPtr hParentWnd, string commonRoot, string[] pathList,
		                                          string originalMessage, int revision)
		{
			try
			{
		...
		string IBugTraqProvider2.ShowOptionsDialog(IntPtr hParentWnd, string parameters)
		{
			try
			{
				var dlg = new OptionUrlEntry(parameters, GetParamDesc());
		...
		private static IEnumerable<string> GetRegistryKeysToAdd()
		{
			return new[]
			{
				@"Implemented Categories\{3494FA92-B139-4730-9591-01135D5E7831}",
			...
			try
			{
				if (typeof(TortoiseSvnPlugin) == t)
				{
					using (var rootKey = Registry.ClassesRoot.CreateSubKey(string.Format(@"CLSID\{0}", CLSID)))
					{
						...
						if (File.Exists(t.Assembly.Location))
						{
							using (RegistryKey key = rootKey.CreateSubKey(@"InprocServer32"))
